# 🎮 Ustyゲーム開発
ゲームでよくある「やられちゃった」の概念を覆したお墓を駆使した2D見下ろし型アクションゲームを開発！完成次第UnityRoomに投稿予定
# 🔥 ゲーム開発計画
- **0211:後期開発発表**
  - 根幹となるコンセプトの実装（完）
- **春休み期間**
  - 休憩
- **2026年前期**  
  - タスク消化
- **2026年後期**
  - 見た目を整えていく。迷わない、世界観がわかる。
# 🔧 開発環境
- **Engine**: ```Unity 6000.2.6f2```
- **Platform**: WebGL
- **Version管理**: Git / GitHub Desktop
- **タスク管理**: issues / Discord 

# 📐 編集ルール
- Membersフォルダーの自分のフォルダーでは**追加・削除・変更**が自由にできます。
- Membersフォルダー以外は、**参照**なら可能です。
- オブジェクトやアセットなどで細かいところで変更して使いたいときにはコピーして自分のファイルに持ってきて使ってください。（後で統合します。）
  - Prefabをコピーして持ってくる。
  - ScriptableObjectを自分のフォルダーで作成する。
  - etc...

**プロジェクトが大きくなっているので相談しながら臨機応変に編集します。**

# 🚂 開発関連リンク
システム関連（github）: https://github.com/wtrinoue/usty_gleap/blob/main/docs/overall.md  
開発計画スライド（Canva) : https://canva.link/et882xr8diyfgrq  
素材役割分担ホワイトボード（Canva）: https://canva.link/fbwm8e977e32r6q

# 🎨 使用アセットについて
スライム：https://assetstore.unity.com/packages/2d/characters/slime-enemy-pixel-art-228568


# 📝 編集前に…
隠しファイルをOnにして、「.git/info/exclude」ファイルにて、
 
<img width="642" height="316" alt="image" src="https://github.com/user-attachments/assets/4ed436b8-3771-4b16-a35b-20e4e584c7b4" /> 


 
*.metaを追加しましょう。 
こうすると.metaファイルの差分はcommitされますが、編集履歴に表示されなくなります。 

.metaファイルは必要ですが編集できないファイルなので、github desktopでは非表示にすると見やすいです。
