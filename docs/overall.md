...
```
- このインターフェースを実際にCoroutineManagerを使いたいクラスに融合して、メソッドをオーバーライド
``` csharp
using UnityEngine;

public class StatusManager : MonoBehaviour, ICoroutineUpdatable
{
    ...
    {
        isDestroyed = true;
        CoroutineManager.Instance.Unregister(this); //←ここで登録することで機能する
    }
    ...
    public void ManagedCoroutine(float interval)
    {
        ...
        // ここに定期処理を書く
        Debug.Log("更新処理: " + interval);
    }
}
```
- CoroutineManagerにオブジェクトを登録すると勝手に処理を回してくれる。
``` csharp
...
public class CoroutineManager : MonoBehaviour
{
    ...
    private List<ICoroutineUpdatable> list = new();
    ...
    IEnumerator Loop()
    {
        while (true)
        {
            ...
            {
                if (item != null)
                    item.ManagedCoroutine(interval);
            }
    ...
    public void Register(ICoroutineUpdatable target)
    {
        if (!list.Contains(target))
            list.Add(target);
    }

    public void Unregister(ICoroutineUpdatable target)
    {
        list.Remove(target);
    }
}
...
## StatusTokenの実装
旧システムの場合はメソッドの中にStatusを入れて、関数内で変更などを行っていたのでStatusの寿命と関数の発動タイミングが合わずにnullになるエラーがあった。なので、StatusTokenというものに情報をコピーして、Statusを変更する側が任意のタイミングで発動するように変更した。このStatusTokenを保持する仕組みにすることで、寿命を示し合わせる必要はないのでエラーがなくなった。
